...
@app.post(
    ...
    response_model=SimpleQualityCheckResponse,
    tags=["Quality Check"],
    summary="Simple quality check - just generated template in, quality report out",
    description="Ultra-simple quality check endpoint that takes a generated ISO template and returns an LLM-generated quality report. No complex typing, guaranteed to work.",
    operation_id="qualityCheckSimple"
)
async def quality_check_simple(request: SimpleQualityCheckRequest):
    """
    Simple quality check endpoint that completely circumvents typing errors.
    
    Takes only a single string (the generated ISO template) as input and returns
    a comprehensive quality report generated by the LLM.
    
    No complex data structures, no validation errors - just pure simplicity.
    """
    try:
        logger.info("Starting simple quality check")
        ...
        # Call the simple LLM quality check
        quality_report = llm_service.simple_quality_check(request.generated_template)
        
        # Return the response
        response = SimpleQualityCheckResponse(
            quality_report=quality_report,
            success=True,
            timestamp=datetime.now().isoformat()
        )
        
        logger.info(f"Simple quality check completed successfully. Report length: {len(quality_report)} chars")
        return response
